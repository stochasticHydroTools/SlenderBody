...
namespace uammd{
  namespace BDHI{
	...
	void Mdot(real3 *Mv, real shearStrainIn, cudaStream_t st);
	...
	*/
    ...
    /*Far contribution of M·F and B·dW
      ...
      = σ·St·FFTi( B·FFTf·S·F + 1/√σ·√B·dWw)
    */
      void FarField::Mdot(real3 *MF, real shearStrainIn, cudaStream_t st){
    sys->log<System::MESSAGE>("[BDHI::PSE] Far shear strain %f", shearStrain);
	...
	// Reset shear strain and re-compute Fourier factors
	shearStrain = shearStrainIn;
	initializeFourierFactors();
	spreadForce(st);
