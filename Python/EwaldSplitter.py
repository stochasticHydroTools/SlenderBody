...
class EwaldSplitter(object):
    ...
    def checkrcut(self):
        ...
        period.
        """
        Lmin = self._Dom.getminLen()/self._Dom.safetyfactor();
    ...
    def EwaldTotalVel(self,Npts,ptsxyz,forces,treeofPoints):
        """
        ...
        Ewaldnear = self.EwaldNearVelkD(Npts,ptsxyz,treeofPoints,forces); # near field Ewald
        #Ewaldnear2 = self.EwaldNearVelQuad(Npts,ptsxyz,forces); # near field Ewald quadratic
    ...
    def EwaldFarVel(self,Npts,ptsxyz,forces):
        ...
        pts = self._Dom.primecoords(ptsxyz);
        g = self._Dom.getg();
    ...
    def EwaldNearVelkD(self,Npts,ptsxyz,treeOfPoints,forces):
        """
        Near field velocity. 
        Inputs: Npts = the number of blobs, ptsxyz = the list of points 
        in undeformed, Cartesian coordinates, forces = forces at those points,
        ...
        #import time
        #t=time.time();
        # print 'Doing Ewald with g=%f' %self._Dom.getg();
        pairpts = self._Dom.kDTreeNeighbors(treeOfPoints, self._rcut);
        Npairs = len(pairpts);
    ...
    def EwaldNearVelQuad(self,Npts,pts,forces):
        """
        Near field velocity. 
        Inputs: Npts = the number of blobs, ptsxyz = the list of points 
        in undeformed, Cartesian coordinates, forces = forces at those points.
        ...
        for iPt in xrange(Npts): # loop over points
            for jPt in xrange(Npts):
                ...
                rvec = self._Dom.calcShifted(pts[iPt,:]-pts[jPt,:]);
    ...
    def EwaldNearVelBins(self,Npts,pts,forces):
        """
        Near field velocity. 
        Inputs: Npts = the number of blobs, ptsxyz = the list of points 
        in undeformed, Cartesian coordinates, forces = forces at those points.
        ...
        nxBin, nyBin, nzBin = self._Dom.calcnBins(self._rcut);
        ptbins = self._Dom.binsbyP(pts,nxBin,nyBin,nzBin);
