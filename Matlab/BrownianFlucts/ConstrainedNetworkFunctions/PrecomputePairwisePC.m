function [PCMats,AllRHSInds,TauStart,NodeOrderByPair] = PrecomputePairwisePC(Xinput,...
    XInvFcn,AllMs,BranchIndices,MasterConnections,SlaveConnections,...
    IntegrationMatrix,RegGridMatrix,RegGridMatrixInv,BendForceMat,impcodt,L,nFib,clampedTau)
    Nxx = length(Xinput);
    Nx = Nxx/(3*nFib);
    ...
    for iC=1:size(MasterConnections,1)
        ...
        AllRHSInds{iC,1}=stAlphInds;

        % Start at the lead nodes
        [Mup,MFUp,GTGUp] = IrregNodeMobility(UpFib,RegGridMatrix,RegGridMatrixInv,...
            [LeadIndUp SlavesOnUp],AllMs,BendForceMat);
        [Mdwn,MFdown,GTGDwn] = IrregNodeMobility(DownFib,RegGridMatrix,RegGridMatrixInv,...
            [LeadIndDown SlavesOnDown],AllMs,BendForceMat);
        ...
        for iS=1:length(MasterLinksPair)
            ...
            [Mex,MFex,GTGex] = IrregNodeMobility(SlaveFib,RegGridMatrix,RegGridMatrixInv,...
                SlaveNode,AllMs,BendForceMat);
            M = blkdiag(M,Mex);
            MF = blkdiag(MF,MFex);
        ...
        Minv = inv(M);
        NMat = pinv(K'*GTG*(Minv*KWithImp));

        % Save preconditioner matrices
        PCMats{iC,1}=Minv;
        PCMats{iC,2}=K;
        ...
        NodeOrderByPair{iC,1}=[LeadIndUp SlavesOnUp];
        NodeOrderByPair{iC,2}=[LeadIndDown SlavesOnDown];
        NodeOrderByPair{iC,3}=MasterLinksPair;

    end
end

function [Mup,MFUp,GTG] = IrregNodeMobility(UpFib,RegGridMatrix,RegGridMatrixInv,...
    inds,AllMs,BendForceMat)
    RegGridUpInv = stackMatrix(RegGridMatrixInv{UpFib});
