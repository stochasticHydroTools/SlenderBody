...
function [KTogether,KTogetherInv] = KWithLink(nFib,N,Nx,Xt,XonNp1Mat,InvXonNp1Mat,LinkInverse)
    ...
    for iFib=1:nFib
        ...
        InvDOFVelocity(FibOmegaInds,FibTauInds) = TauCross;
    end
    % The matrix for the links to evolve
    OmegaInv = zeros(3*Nlinks);
    for iLink=1:Nlinks
        ...
        OmegaInv((iLink-1)*3+1:3*iLink,(iLink-1)*3+1:3*iLink)=CPMatR/norm(RLink)^2;
    end
    LinkInverse(3*Nlinks+1:end,:)=OmegaInv*LinkInverse(3*Nlinks+1:end,:);
    InvDOFVelocity(6*N+1:end,:)=LinkInverse;
    KTogether = blkdiag(XonNp1Mat,XonNp1Mat)*DOFVelocity;
    KTogetherInv = InvDOFVelocity*blkdiag(InvXonNp1Mat,InvXonNp1Mat);
end
