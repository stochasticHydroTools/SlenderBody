...
%
L=2;
[s0,w0]=chebpts(16,[0 2],1);
LamLoc=load('Lambdas.txt');
LamH=load('LambdasHYDRO.txt');
LamLoc=reshape(LamLoc,3,16*700)';
LamH=reshape(LamH,3,16*700)';
LamNorms=zeros(700,2);
for iFib=1:700
    inds=(iFib-1)*16+1:iFib*16;
    LamNorms(iFib,1)=sqrt(w0*sum(LamLoc(inds,:).*LamLoc(inds,:),2));
    LamNorms(iFib,2)=sqrt(w0*sum(LamH(inds,:).*LamH(inds,:),2));
end
% Plot the fibers
plotfibs=234;
plotlinks=[];    
links=load('F700C8400.txt');
for iFib=plotfibs
    plotlinks = [plotlinks; find(floor(links(:,1)/16)==iFib); find(floor(links(:,2)/16)==iFib)];
end
% Compute lambdas for those fibers
Fibs=unique([floor(links(plotlinks,1)/16);floor(links(plotlinks,2)/16)])+1;
lNorms = LamNorms(Fibs,:);
nzation = max(lNorms(:));
colindex = ceil(lNorms/nzation*1000);
cm =colormap(hsv(1000));
LocalColors = cm(colindex(:,1),:);
NLColors = cm(colindex(:,2),:);
