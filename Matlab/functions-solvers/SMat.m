...
function SWeights = SMat(xEpts,yEpts,zEpts,IBpts,sup,gw)
    ...
    for ilam=1:NIB
        ...
        zclose=zEpts(floorz)+round(IBpts(ilam,3)/Lz)*Lz;
        ...
        floory=mod(floor((IBpts(ilam,2)-aey)/hey),Ny)+1;
        yclose=yEpts(floory)+round(IBpts(ilam,2)/Ly)*Ly;
        ...
        floorx=mod(floor((IBpts(ilam,1)-aex)/hex),Nx)+1;
        xclose=xEpts(floorx)+round(IBpts(ilam,1)/Lx)*Lx;
