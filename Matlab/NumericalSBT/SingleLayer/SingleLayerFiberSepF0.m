...
for iN=1:N
    for iT=1:Nthet
        ...
        MConstf(3*iindex-2:3*iindex,:) = IntegrateOutTheta(X,n1,n2,radius,Nthet,N,s,w,iN,iT,se,efflength,c,Xs,mu);
        for jN=1:N
            for jT=1:Nthet
        ...
        MConstf(3*iindex-2:3*iindex,3*iN-2:3*iN)=MConstf(3*iindex-2:3*iindex,3*iN-2:3*iN)+Ans2;
    end
end
% Solve system for (f0, df), where sum(df) = 0 around each cross section
% Matrix mapping f0 and df to traction
Assignf0 = zeros(3*Nt,3*N);
for iN=1:N
    for iT=1:Nthet
        iindex = (iN-1)*Nthet+iT;
        for iD=-2:0
            Assignf0(3*iindex+iD,3*iN+iD)=1/(2*pi);
        end
    end
end      
...
A = [MConstf/(2*pi) M; zeros(3*N) Assignf0'];
fandfbar = A \ [reshape(U3',3*Nt,1); zeros(3*N,1)];
max(abs(reshape(fandfbar(1:3*N),3,N)'-favg))
favg = reshape(fandfbar(1:3*N),3,N)';
...
function TotalInts = IntegrateOutTheta(X,n1,n2,radius,Nthet,N,s,w,iN,iT,se,efflength,c,Xs,mu)
    syms tpr
    % Generate 3N x N matrix relating constant force to velocity
    % Here we integrating over theta' in the Stokeslet to high accuracy
    TotalInts = zeros(3,3*N);
    itheta = iT/Nthet*2*pi;
    iRhoHat = n1(iN,:)*cos(itheta)+n2(iN,:)*sin(itheta);
    sprime = se;
    Xisph = efflength*(sprime-se)*Xs(iN,:)+c*sqrt(1-sprime^2)*iRhoHat+X(iN,:);
    for jN=1:N
        prRhoHat = n1(jN,:)*cos(tpr)+n2(jN,:)*sin(tpr);
        dX = X(iN,:)+radius(iN)*iRhoHat-(X(jN,:)+radius(jN)*prRhoHat);
        sprime = s(jN);
        Xjsph = efflength*(sprime-se)*Xs(iN,:)+c*sqrt(1-sprime^2)*prRhoHat+X(iN,:);
        dXe = Xisph-Xjsph;
        constpart = X(iN,:)-X(jN,:)+radius(iN)*iRhoHat;
        cospart = -radius(jN)*n1(jN,:);
        sinpart = -radius(jN)*n2(jN,:);
        constpart_e = Xisph - (efflength*(sprime-se)*Xs(iN,:)+X(iN,:));
        cospart_e = -c*sqrt(1-sprime^2)*n1(jN,:);
        sinpart_e = -c*sqrt(1-sprime^2)*n2(jN,:);
        Rsq_cons = dot(constpart,constpart)+radius(jN)^2;
        Rsq_cos = 2*dot(constpart,cospart);
        Rsq_sin = 2*dot(constpart,sinpart);
        Rsq_cons_e = dot(constpart_e,constpart_e)+c^2*(1-sprime^2);
        Rsq_cos_e = 2*dot(constpart_e,cospart_e);
        Rsq_sin_e = 2*dot(constpart_e,sinpart_e);
        % Evaluate numerically at Cheb pts, set singular point to zero
        Ntpr=100;
        dthpr = 2*pi/Ntpr;
        thpr=(0:Ntpr-1)*dthpr+itheta;
        Rsq_1 = Rsq_cons+Rsq_cos*cos(thpr)+Rsq_sin*sin(thpr);
        Rsq_e = Rsq_cons_e+Rsq_cos_e*cos(thpr)+Rsq_sin_e*sin(thpr);
        RinvIG_1 = 1./sqrt(Rsq_1);
        RinvIG_E = 1./sqrt(Rsq_e);
        if (iN==jN)
            RinvIG_E(1)=0;
            RinvIG_1(1)=0;
        end
        OneOverRIntegral_1 = dthpr*sum(RinvIG_1);
        OneOverRIntegral_E = dthpr*sum(RinvIG_E);
        RRtOverR3Int_1 = zeros(3);
        RRtOverR3Int_E = zeros(3);
        for jT=1:Ntpr
            if (iN==jN && jT==1)
            else
            R = constpart+cospart*cos(thpr(jT))+sinpart*sin(thpr(jT));
            Re = constpart_e+cospart_e*cos(thpr(jT))+sinpart_e*sin(thpr(jT));
            RRtOverR3Int_1 = RRtOverR3Int_1 + (R'*R)/norm(R)^3*dthpr;
            RRtOverR3Int_E = RRtOverR3Int_E + (Re'*Re)/norm(Re)^3*dthpr;
            end
        end
        TotalInts(:,3*jN-2:3*jN) = TotalInts(:,3*jN-2:3*jN)...
            +1/(8*pi*mu)*(OneOverRIntegral_1*eye(3)+RRtOverR3Int_1)*w(jN);
        TotalInts(:,3*iN-2:3*iN) = TotalInts(:,3*iN-2:3*iN)...
            -1/(8*pi*mu)*(OneOverRIntegral_E*eye(3)+RRtOverR3Int_E)*w(jN);
    end
end
