...
[sDouble, wDouble, bDouble] = chebpts(2*N, [0 L], 1);
Ddouble = diffmat(2*N,[0 L],'chebkind1');
IntDdoublest = stackMatrix(pinv(Ddouble));
...
RNToDouble = barymat(sDouble,s,b);
RNToDouble_st = stackMatrix(RNToDouble);
W2 = diag(wDouble);
BendingEnergyMatrix = stackMatrix((RNToDouble*D^2)'*W2*RNToDouble*D^2);
WtildeInverse = stackMatrix((RNToDouble'*W2*RNToDouble)^(-1));
BendingForceMatrix = -Eb*WtildeInverse*BendingEnergyMatrix;
