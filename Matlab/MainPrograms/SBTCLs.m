...
makeMovie=1;
nFib=25;
nCL = 25;%nFib*(nFib-1)/2;
N=16;
Lf=2;   % microns
nonLocal = 1; % whether to do the nonlocal terms 
maxiters = 1;
Ld = 4; % periodic domain size (microns)
...
rl = 0.5; % rest length for cross linkers
...
PyLocs=load('FreeSpaceLocsF25C25REV.txt');
fibpts=PyLocs(1:nFib*N,:);
% fibpts=PyLocs(997*N+1:998*N,:);
% fibpts = [zeros(N,1) s0-Lf/2 zeros(N,1); zeros(N,1)+0.3 Lf/2-s0 zeros(N,1);...
%    zeros(N,1)-0.3 s0-Lf/2 zeros(N,1); zeros(N,1)+0.6 s0-Lf/2 zeros(N,1)];
% fibpts=initFibs(0,Ld,nFib,s0);
...
Xpts=[];
forces=[];
links=makeCLinks([],fibpts,nFib,N,rl,Lf,nCL,Ld,0,0);
%links = [1 Lf/2 2 Lf/2 0 0 0];
