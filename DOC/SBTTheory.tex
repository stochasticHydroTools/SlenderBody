...
\end{equation}
for some scalar function $T$. Differentiating both sides of Eq.\ \eqref{eq:tsalm} with respect to $s$, we obtain 
\begin{equation}
\label{eq:lamval}
...
\end{equation}
Now, let $c=\bm{X}_s(L) \cdot \bm{X}_s(0)$. Then we can take the inner product of $\bm{X}_s(L)$ and $\bm{X}_s(0)$ with Eq.\ \eqref{eq:BCT} to obtain
\begin{gather}
T(L) - cT(0)=0 \qquad cT(L)-T(0)=0. 
\end{gather}
If $|c|\neq1$, then we obtain $T(0)=T(L)=0$, which is exactly the boundary condition for the line tension equation in Tornberg and Shelley. If $|c|=1$,  $T(0)=T(L)=$ constant, which can be set to 0 since $T$ only appears differentiated in the equations. And so in all cases we recover the boundary condition $T(s=0, L)=0$ of Tornberg and Shelley \cite{ts04}. 
