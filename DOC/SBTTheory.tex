...
Let us summarize the equations of motion in the weak form most suitable to stochastic evolution on a space of inextensible worm-like chains. We add here Brownian noise following \cite{FluctuatingFibers_Saintillan_PNAS,FluctuatingFibers_Saintillan_PF}. Recall that the process begins by choosing a set of basis functions $g_k(s)$ for $L_2:[0,s]$, which in the discrete formulation we take to be Chebyshev polynomials but this is not required. In this part we focus on a single fiber and we will ignore the translational motion of the fiber, which means we will set $\V{U}= \frac{\partial\V{X}}{\partial t}(s=0) = \V{0}$ as this is the easy part.

In order to avoid constraints, we will write here the evolution for the tangent vector along the fiber $\V{X}_s(\theta(s,t),\phi(s,t)) = \left( \cos{\theta} \cos{\phi},\, \sin{\theta} \cos{\phi},\,  \sin{\phi} \right)$ instead of $\V{X}$, but the inextensibility constraint will be implicit in the dynamical equations which arise from enforcing it with a tension Lagrange multiplier.

First, the Lagrange multiplier $\lambda(s)$ and direction of motion $\bm{\alpha}=(\alpha_{1k},\alpha_{2k})$ (here $k$ is an index counting the basis functions) in the tangent to the manifold of inextensible fibers are computed by solving the saddle-point system
\begin{equation}
...
\bm{0}\end{pmatrix},
\end{equation}
where $\V{\zeta}(s,t)$ is white noise in space and time. In the semi-continuum weak formulation, $\bm{\alpha}$ is an infinite dimensional vector of countably-many expansion coefficients, while $\lambda(s)$ is a function or distribution. Recall here that $\bm{L}\V{X}=-E \V{X}_{ssss}$ but taking into account the boundary conditions $\V{X}_{ss}=\V{X}_{sss}=0$ at both free ends (how does one handle this in the context of fluctuating fibers?!?). 

For the local drag model $\bm{M}=\bm{\Lambda}/(8\pi\mu)$, an explicit formula for the Brownian force density can be written (personal communication with David Saintillan):
\begin{equation}
\V{f}^B(s,t) = \sqrt{\frac{2 k_B T}{8\pi\mu}} \left(\bm{\Lambda}^{\frac{1}{2}} \V{\zeta}\right)(s,t) = \left( c_1(s) \M{I} + c_2(s) \V{X}_s \V{X}_s \right) \V{\zeta}(s,t),
\end{equation}
where the form of $c_1(s)$ and $c_2(s)$ can be computed by computing the covariance of $\V{f}^B$ and comparing it against \eqref{eq:c_t_general}. But this form should not be required in the formulation of the algorithm, which should also work even when the mobility includes non-local parts (which should be symmetric positive-definite also though this is not obvious just by looking at \eqref{eq:intsbt}).

The dynamics of the fiber tangent vector is then given by 
\begin{equation}
\label{eq:Xsupdate_fluct}
\V{X}_{st} = \V{\Omega}\left(\V{X}_s, \V{\alpha}\right) \times \V{X}_s, 
\end{equation}
where
\begin{equation}
\V{\Omega}\left(\V{X}_s, \V{\alpha}\right)=\sum_k \alpha_{2k} g_k(s) \V{n_2}(s) - g_2(s)\V{n}_1
\end{equation}
