function [ut,lambdas,fE] = ExSolve(nFib,N,Ms,Ks,Kts,I,wIt,FE,LRLs,URLs,Xt,Xs,...
                                U0,s0,w0,Lf,epsilon,D,mu,xi,L,nonLocal,lamprev,maxiters,grav,fext)  
    ...
    while ((nonLocal || iters==0) && reler > 1e-6 && iters < maxiters)
        if (nonLocal)
            nLvel = MNonLocalSlow(nFib,N,s0,w0,Lf,epsilon,reshape(fE+l_m+fext,3,N*nFib),...
                     reshape(Xt,3,N*nFib)',reshape(Xs,3,N*nFib)',D(1:3:3*N,1:3:3*N),mu);
        ...
        for iFib=1:nFib
            ...
            ut(inds) = K*alphas+I*Urigid;
            l_m1(inds) = l_m(inds);
            l_m(inds) = M \ (B*alphaU-M*(fE(inds)+fext(inds))-U0(inds)-nLvel(inds));
        end
